    ...
    while i < len(validators):
        ...
        while j < num_concurrent:
            ...
            subprocess.check_output(
                f"ssh -i {ssh_key} ubuntu@{validators[i + j]} 'sudo -S -p \"\" systemctl restart wled'")
            j += 1
        i += num_concurrent
...
def delegation_change(chain_id):
    ...
    wled_query_cmd = f"wled q staking delegations {admin_acc} --chain-id {chain_id} --output json"
    staking_output = json.loads(
    subprocess.check_output([wled_query_cmd], stderr=subprocess.STDOUT,
                                shell=True))
    total_delegation = 0
    validator_addrs = []
    ...
    for i in range(len(validator_addrs // 4)):
        ...
        wled_staking_cmd = f"wled tx staking delegate {validator_addrs[i]} {10 * total_delegation}uwle --from admin --chain-id {chain_id} -b block -y"
        _run_wled_cmd(wled_staking_cmd)
    time.sleep(random.randint(600, 3600))
    # Unbond
    for validator in delegations:
        print(f"Unbonding {10 * total_delegation} from {validator}")
        wled_unbond_cmd = f"wled tx staking unbond {validator} {delegations[validator]}uwle --from admin -b block -y --chain-id {chain_id}"
        _run_wled_cmd(wled_unbond_cmd)
...
def _get_admin_acc():
    wled_query_cmd = "wled keys list --output json"
    accs_output = _run_wled_cmd(wled_query_cmd)
    return filter(lambda x: x['name'] == 'admin', accs_output)['address']

def _run_wled_cmd(cmd)
    wled_cmd = "printf '12345678\n' " + cmd
    output = json.loads(
        subprocess.check_output([wled_cmd], stderr=subprocess.STDOUT,
                                shell=True))
    return output
